...
impl Layout {
    ...
    pub fn vertex_position(&self, topo: &Topology, v: VertexId) -> (f32, f32){
        let (x, y) = topo.vertex_hexes()[v.value()].iter()
            .map(|h| self.hex_to_pixel(*h))
            .fold((0.0, 0.0), |(ax, ay), (hx, hy)| (ax + hx, ay + hy));
        (x / 3.0, y / 3.0)
    }

    pub fn edge_endpoints(&self, topo: &Topology, e: EdgeId) -> ((f32, f32), (f32, f32)){
...
mod tests {
    ...
    fn les_sommets_sont_a_hex_size_du_centre_de_leur_tuile() {
        ...
        let layout = Layout { hex_size: 40.0, origin: (0.0, 0.0) };
        ...
        for (i, vertices) in topo.tile_vertices().iter().enumerate() {
            ...
            for &v in vertices {
                ...
                assert!((d - 40.0).abs() < 0.01, "sommet {v:?} à {d} du centre de la tuile {i}");
            }
        }
    }
